...
public class BattleSeaPlayPageController implements Initializable {
			...
			if (!bombThrown) {
				...
				if (opponentPlayer.shipExistsInXY(getXFrom1(index), getYFrom1(index))) {
					...
					if (shipToBeBombed.isDestroyed(opponentPlayer)) {
						Image explosionSprite = new Image(String.valueOf(MainController.setImageFromFile("src/com/Resources/Images/explosion.png")));
	...
	private Label getBombLabel (Bomb bomb, boolean isForBigBoard) {
		return new Label() {{
			String imgUrl = (bomb.wasSuccessful() ? String.valueOf(MainController.setImageFromFile("src/com/Resources/Images/successfulBomb.png")) : String.valueOf(MainController.setImageFromFile("src/com/Resources/Images/redX.png")));
